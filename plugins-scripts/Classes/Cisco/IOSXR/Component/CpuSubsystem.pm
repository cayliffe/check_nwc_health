package Classes::Cisco::IOSXR::Component::CpuSubsystem;
...
sub init {
  ...
  foreach ($self->get_snmp_table_objects(
    ...
    push(@{$self->{cpus}},
        Classes::Cisco::IOSXR::Component::CpuSubsystem::Cpu->new(%{$_}));
  }
  if (scalar(@{$self->{cpus}}) == 0) {
    ...
    if (exists $response->{$Classes::Device::mibs_and_oids->{'OLD-CISCO-CPU-MIB'}->{avgBusy1}}) {
      push(@{$self->{cpus}},
          Classes::Cisco::IOSXR::Component::CpuSubsystem::Cpu->new(
...
package Classes::Cisco::IOSXR::Component::CpuSubsystem::Cpu;
